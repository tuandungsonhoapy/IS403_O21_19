...
[6] Mô hình thuật toán Linear Regression được sử dụng để dự đoán giá trị của một biến phụ thuộc (biến đầu ra) dựa trên giá trị của một hoặc nhiều biến độc lập (biến đầu vào), được sử dụng để xác định và phân tích các xu hướng trong dữ liệu thời gian. Trong bài báo này, hiệu suất của hồi quy tuyến tính (LR) và hồi quy vectơ hỗ trợ (SVR) để dự đoán giá cổ phiếu của Amazon vào tháng 10 năm 2019. Phát hiện của họ chỉ ra rằng LR đạt được độ chính xác cao hơn (\(98,76\%\)) so với SVR (\(94,32\%\)), cho thấy khả năng phù hợp của nó trong ngắn hạn - giá kỳ hạn trước khi xác định cổ phiếu có ít biến động hơn.
...
\begin{itemize}
    ...
    \item Dễ triển khai và tính toán.
    \item Phù hợp cho dữ liệu có mối quan hệ tuyến tính đơn giản.
    \item Có thể được mở rộng để xử lý các mô hình phức tạp hơn thông qua các biến phụ thuộc phi tuyến tính hoặc sử dụng kỹ thuật biến đổi dữ liệu.
\end{itemize}
...
Trong đó:
\begin{itemize}
    \item \( Y \): Đây là biến phụ thuộc (hoặc biến mục tiêu), là giá trị chúng ta muốn dự đoán hoặc giải thích bằng các biến độc lập khác.
    \item \( X_1, X_2, \ldots, X_p \): Là các biến độc lập, là các đặc trưng hoặc biến giải thích mà chúng ta sử dụng để dự đoán giá trị của \( Y \).
    \item \( \beta_0 \): Là hệ số chặn của đường hồi quy, thường được gọi là hệ số chặn hoặc hệ số độc lập. Nó cho biết giá trị của \( Y \) khi tất cả các biến độc lập đều bằng 0.
    \item \( \beta_1, \beta_2, \ldots, \beta_p \): Là các hệ số hồi quy của các biến độc lập tương ứng \( X_1, X_2, \ldots, X_p \). Chúng cho biết mức độ ảnh hưởng của mỗi biến độc lập lên biến phụ thuộc \( Y \).
    \item \( \epsilon \): Là sai số ngẫu nhiên, biểu thị những yếu tố không được mô hình hóa hoặc không biết đến có thể ảnh hưởng đến biến phụ thuộc \( Y \).
\end{itemize}
...
[6] D. Bhuriya, G. Kaushal, A. Sharma, and U. Singh, “Stock market predication using a linear regression,” in 2017 International conference of Electronics, Communication and Aerospace Technology (ICECA),
